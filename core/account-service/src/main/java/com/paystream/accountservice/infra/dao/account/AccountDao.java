    ...
    public void insert(Account a) {
        String sql =
                "INSERT INTO accounts (id, customer_id, currency, status, version, created_at, updated_at) " +
                        "VALUES (?, ?, ?, ?, 0, now(), now())";
        jdbc.update(sql, a.id(), a.customerId(), a.currency(), a.status());
    }

    public Optional<Account> findById(UUID id) {
        String sql =
                "SELECT id, customer_id, currency, status, version, created_at, updated_at FROM accounts WHERE id = ?";
        return jdbc.query(sql, RowMappers.ACCOUNT, id).stream().findFirst();
    }
}
